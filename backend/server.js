...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    res.status(201).json({ msg: 'User created successfully', user: newUser.rows[0] });
...
app.get('/api/test/keeper', [auth, authorize('Keeper', 'Admin')], (req, res) => {
  res.json({
...
app.post('/api/items', [auth, authorize('Admin', 'Keeper')], async (req, res) => {
  try {
    const { sku, name, description, category_id, reorder_level, attributes } = req.body;
    ...
    if (initial_stock > 0) {
      ...
      const updatedItemQuery = await pool.query("SELECT * FROM Items WHERE id = $1", [createdItem.id]);
...
// @access  Private (Admin, Keeper)
app.put('/api/items/:id', [auth, authorize('Admin', 'Keeper')], async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, category_id, reorder_level, attributes } = req.body;
...
app.post('/api/transactions', auth, async (req, res) => {
  try {
    ...
    if (!item_id || !type || !quantity) {
      return res.status(400).json({ msg: 'Please provide item_id, type, and quantity' });
    }
    ...
    if (type === 'OUTBOUND') {
      const itemQuery = await pool.query("SELECT current_stock FROM Items WHERE id = $1", [item_id]);
